...
static void SDLCALL AndroidCameraPermissionCallback(void *userdata, const char *permission, bool granted)
{
    SDL_Log("Camera permission callback: %s", granted ? "GRANTED" : "DENIED");
...
static absl::Status TestMediaPipe()
{
...
//    const char *camera_name = "FaceTime HD Camera";
    ...
    if (!state) {
        SDL_Log("ERROR: SDLTest_CommonCreateState failed: %s", SDL_GetError());
        return SDL_APP_FAILURE;
    }
    SDL_Log("SDLTest_CommonCreateState succeeded");
    ...
    SDL_Log("SDLTest_CommonInit succeeded");
...
#ifdef __ANDROID__
    SDL_Log("Requesting Android CAMERA permission...");
    if (!SDL_RequestAndroidPermission("android.permission.CAMERA", AndroidCameraPermissionCallback, NULL)) {
        SDL_Log("ERROR: Failed to submit camera permission request: %s", SDL_GetError());
    ...
    SDL_Log("Camera permission request submitted successfully");
    
    /* Wait briefly for the permission callback */
    SDL_Delay(100);
    
    if (!camera_permission_granted) {
        SDL_Log("Camera permission not granted - cameras may not be available");
        /* Don't fail here - let it try anyway, the permission dialog may still be showing */
    }
#endif

    SDL_Log("Using SDL camera driver: %s", SDL_GetCurrentCameraDriver());

    SDL_Log("Calling SDL_GetCameras...");
    SDL_CameraID *devices = SDL_GetCameras(&devcount);
    if (!devices) {
        SDL_Log("ERROR: SDL_GetCameras failed: %s", SDL_GetError());
        return SDL_APP_FAILURE;
    }
    SDL_Log("SDL_GetCameras succeeded");
    ...
    if (devcount == 0) {
        SDL_Log("WARNING: No cameras found on this device!");
        SDL_free(devices);
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "No Camera", "No cameras detected on this device!", window);
        return SDL_APP_FAILURE;
    }
...
//        }
    ...
    if (!camera_id) {
        SDL_Log("ERROR: No cameras available?");
        return SDL_APP_FAILURE;
    }

    SDL_Log("Selected camera_id: %u", camera_id);
    
    PickCameraSpec(camera_id, &spec);
    
    /* Log the spec we're trying to use */
    SDL_Log("Opening camera with spec: %dx%d %.2f FPS %s", 
            spec.width, spec.height, 
            spec.framerate_denominator ? (float)spec.framerate_numerator / spec.framerate_denominator : 0.0f,
            SDL_GetPixelFormatName(spec.format));
    
    /* If we couldn't find a suitable spec, try with NULL to let SDL pick */
    if (spec.width == 0 || spec.height == 0) {
        SDL_Log("No suitable spec found, letting SDL choose defaults");
    ...
    if (!camera) {
        SDL_Log("ERROR: Failed to open camera device: %s", SDL_GetError());
        return SDL_APP_FAILURE;
    }
